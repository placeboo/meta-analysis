...
do.one.simulation = function(n, n1, gamma.true, beta.true, eta.true){
     ...
     if (is.matrix(test2)) {
          test1 = rep(NA, 8)
     } else test1 = test1$coefficients[,c(1,2)]
     if (is.matrix(test2)) {
          test2 = rep(NA, 8)
     } else test2 = test2$coefficients[,c(1,2)]
     return(c(test1, test2))
}
N_sim = 100

est.mat = replicate(N_sim, do.one.simulation(n, n1, gamma.true, beta.true, eta.true))

index = unique(unlist(apply(est.mat, 1, function(x) which(is.na(x)))))

if (length(index) > 0){
     est.mat = est.mat[,-index]
     est.tmp = replicate(length(index), do.one.simulation(n, n1, gamma.true, beta.true, eta.true))
...
save(est.mat, file = paste("case-control2/casecontrol", n, "_seed", seed, "_", st, ".RData", sep = ""))
