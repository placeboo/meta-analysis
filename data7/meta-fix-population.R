...
do.one.simulation = function(n1, n2, pr, gamma.true, beta.true, eta.true) {
        ...
        if (n2 == 0) {
                return(c(test1$coefficients[,c(1,2)]))
        } else {
                ...
                opt = c(test.all$coefficients[, c(1,2)])
...
est.mat = replicate(N_sim, do.one.simulation(n1, n2, pr = 1/3,  gamma.true, beta.true, eta.true))

index = unique(unlist(apply(est.mat, 1, function(x) which(is.na(x)))))

while (length(index) > 0){
        est.mat = est.mat[,-index]
        est.tmp = replicate(length(index), do.one.simulation(n1, n2, pr = 1/3,  gamma.true, beta.true, eta.true))
...
save(est.mat, file = paste("data7/crt", n1, "_casecrt", n2, "_seed", seed, "_", st, ".RData", sep = ""))
